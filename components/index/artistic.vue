...
export default {
  ...
  async mounted(){//hover时才有数据 所以先在mounted加载数据
    let self=this;
  ...
  methods: {
    over: async function (e) {
      ...
      if (tag === 'dd') {
        ...
        if(status===200&&count>0){
          let r= pois.filter(item=>item.photos.length).map(item=>{//过滤以后用map修改数据格式 map映射
            return {//前端需要的数据结构和后端不一致时:
              title:item.name,
              pos:item.type.split(';')[0],
              price:item.biz_ext.cost||'暂无',//降级校验
              img:item.photos[0].url,
              url:'//abc.com'
            }
          })
